...
def _request_dumb_move(
    ...
    *,
    ...
    return client.post(
        ENGINE_MOVE_URL,
        json={"fen": fen, "engine": "dumb", "metadata": metadata},
    )


def test_dumb_move_response_contract_for_both_colors(
    client: TestClient,
    ...
    response = _request_dumb_move(client, fen)

    assert response.status_code == 200
    body = response.json()
    assert body["move"] in legal_moves
    assert body["engine"] == "Dumb Engine"
    assert body["played_color"] == color
    assert body["metadata"] == {"seed": 17, "depth": 1}
...
def test_dumb_move_is_deterministic_for_both_colors(
    client: TestClient,
    engine_position: tuple[str, str],
):
    _, fen = engine_position

    first = _request_dumb_move(client, fen, seed=29)
    second = _request_dumb_move(client, fen, seed=29)
...
def test_dumb_move_accepts_omitted_seed_for_both_colors(
    client: TestClient,
    engine_position: tuple[str, str],
):
    color, fen = engine_position

    response = _request_dumb_move(client, fen, seed=None)

    assert response.status_code == 200
    body = response.json()
    assert body["played_color"] == color
    assert body["move"] is not None
    assert body["metadata"] == {"seed": None, "depth": 1}


def test_dumb_move_returns_domain_error_when_no_move_exists_for_both_colors(
    client: TestClient,
...
def test_dumb_move_requires_depth_for_both_colors(
    client: TestClient,
    engine_position: tuple[str, str],
):
    _, fen = engine_position
    ...
    response = client.post(
        ENGINE_MOVE_URL,
        json={"fen": fen, "engine": "dumb", "metadata": {}},
    )

    assert response.status_code == 400
    assert response.json() == {"detail": "metadata.depth is required"}


def test_dumb_move_rejects_invalid_depth_for_both_colors(
    client: TestClient,
    engine_position: tuple[str, str],
):
    _, fen = engine_position
...
def test_dumb_move_rejects_invalid_seed_for_both_colors(
    client: TestClient,
    engine_position: tuple[str, str],
):
    _, fen = engine_position
...
def test_dumb_move_rejects_invalid_fen_for_both_colors(
    client: TestClient,
    engine_position: tuple[str, str],
):
    ...
    response = _request_dumb_move(client, invalid_fen)

    assert response.status_code == 400
    assert response.json() == {"detail": "Invalid board layout"}


def test_dumb_move_rejects_unknown_metadata_for_both_colors(
    client: TestClient,
    engine_position: tuple[str, str],
):
    _, fen = engine_position
    ...
    assert response.json() == {
        "detail": "Unknown metadata field(s) for dumb: extra, typo"
    }
